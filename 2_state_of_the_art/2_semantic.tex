...
Each triple is composed by a subject, a predicate and an object like in a normal sentence (see Figure~XX).
In that way, in Berners Lee et al. words \emph{a document can assert that things (people, web pages or whatever) have properties (such as "is a sister of," "is the author of") with certain values (another person, another Web page)}.
A key difference with a normal sentence is that each element is usually unambiguously defined by an URI.
These URIs form links between different triples as shown by the example of the Figure~XX.
% This set of triples can be expressed in multiple formats such as RDF, Turtle, Notation3 or NTriples. % TODO citas

% Figura/figuras de tripletas
