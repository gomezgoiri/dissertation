...
\footnote{
  We have followed the convention of Tuple Spaces' seminal paper \cite{gelernter_generative_1985}. Please note that:
  \begin{enumerate}
...
\begin{itemize}
 ...
 \item \emph{Time uncoupling}, which happens when senders and receivers do not need to exists in the same time\footnote{
	  Some authors \cite{fensel_triple-space_2004,krummenacher_www_2005} interchangeably use this property and asynchronous communication.
	  ...
	  However, Coulouris et al.\cite{coulouris_distributed_2012} make a clear distinction between them.
	  In their words, a communication is asynchronous when \emph{a sender sends a message and then continues without blocking}.
	  Time uncoupling adds an extra dimension: \emph{the sender and the receiver can have independent existences}.
	  }.
