...
However, a triple by itself cannot express much information.
...
Usually, the result of a \ac{ts} primitive are all the triples belonging to the same graph.
In other words, the basic information resource in these middlewares is the \ac{rdf} graph and the \ac{rdf} triple is used to ensure the associative access. % URI al final no deja de ser por dirección
TSC presents an exception
Therefore, \ac{rdf} graph adds a higher lever to access to the knowledge.
