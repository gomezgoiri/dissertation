...
In addition, they provide access to the \ac{rdf} graphs by their \acp{uri}.
...
In addition, it offers a mechanism to overlap spaces called \emph{scopes}.
...
% Fensel2007tsc says that they break this with an hybrid architecture.
% The point here would be to decide whether the distribution is introducced by an upper service such as the Triple store or by tuple space middleware
% (analagously, think of a web architecture with a distributed DB at the application level).
The only exception is \midtscpp{}, where each participant of the space is a peer in a \ac{p2p} network.
However, when the subscription/notification systems are offered as part of \ac{tsc}, the \ac{restcs} property is certainly affected.
% Similarly, people argue that AJAX or WebSockets might be a concern to achieve REST architectures.
% http://www.infoq.com/news/2012/02/websockets-rest
% http://savas.me/2005/08/why-i-think-ajax-breaks-rest/
Similarly, the statefulness of these middlewares conflicts with the transactionality feature.
% This is certainly a hard issue to think of, at which level is the transactionality offered? I assume that at the middleware level.
%     http://stackoverflow.com/questions/3105296/if-rest-applications-are-supposed-to-be-stateless-how-do-you-manage-sessions
% It could be build uppon a REST architecture, just as HTTP is build on top of a stateful protocol, TCP on a stateless (IP), etc.
...
In the proposed middleware they will be able not only to write and read knowledge into an external space, but also to enrich it providing their own managed data. % esto puede sonar a que se rompe CS 0:-)
...
\input{\pathchapthree/tables/summary_own}