...
% TODO IG poner el nombrecico y tal
This section presents the first technique to change the physical environment.
This technique is space-based, i.e. participants coordinate by reading and writing in a shared \Space{}.
This encourages an uncoupled communication between applications using the same \Space{}.

% TODO IG aqui ninguno me guia en el famoso "Space actuation". A ver si puedes hacer que se oriente a esto. Imagino que el primero, por ejemplo, intenta presentar la base de como se actua, pero no me entra porque presentas esto asi.
...
Section~\ref{sec:envisaged_scenarios} illustrates with two sample scenarios how to apply some of these patterns to \ac{ubicomp} environments.
Section~\ref{sec:notification} presents an additional requirement identified on these scenarios: a subscription mechanism for the \Space{}.
...
Regarding the application of this patterns to \ac{ubicomp}, we observed that:
...
Consequently, we believe that the \emph{specialist patterns} fit the best to \ac{ubicomp}'s needs.
...
For example, a device might show a message whenever a new warning is written in the space. % e.g. a task or a result
