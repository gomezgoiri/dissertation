...
On the one hand, the reason for the first is that \ac{ts}-based patterns require all the participants to be aware of what is written into the space to react (i.e., be proactive).
...
On the other hand, \restActuation{} requires the consumer to have prior knowledge about the environment to reason over it. % frase susceptible de simplificar
...
\InsertFig{time_by_techniques}{fig:timeTechnique}{Time needed to make a change in per technique}{}{0.7}{}
...
Furthermore, consumers and providers in \spaceActuation{} only perform trivial computing task: interpreting results. % pattern matching sólo en el caso del coordination space!
...
However, note that due to the prototype nature of the subscription mechanism implemented, there is still room for a great performance improvement. % prototype, prototyping?
...
Consequently, from the data consumer perspective, this approach will be generally more energy demanding than the first one. % might porque no está medido, pero es algo intuitivo
