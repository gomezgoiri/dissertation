...
These \acp{api} expose the capabilities of some devices to make physical changes in the environment.

% TODO IG esta intro me gusta mas que la anterior... que sale un poco de la nada.
...
% TODO IG el primero deberia relacionarse con el direct consumption. Entiendo que es backgorund, pero como se relacion con el direct? De hecho en el 6.1.1 quiza tambien deberia llamarse background.
Section~\ref{sec:actuation_rest_background} briefly presents different alternatives to build semantic \ac{rest} \acp{api}.
Section~\ref{sec:restdesc} present the selected alternative and Section~\ref{sec:actuation_scn2} implements the baseline implementation using it.
