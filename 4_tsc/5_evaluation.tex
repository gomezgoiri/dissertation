...
Therefore, we will consider the challenges identified by the \emph{Internet-of-Things Architecture} European project \citep{walewski_project_2011} to analyze our solution.
...
\begin{itemize} % posiblemente incluir lo que se explica aquí en otro punto
  ...
  \begin{enumerate}
    \item In Section~\ref{sec:network_properties} we detailed how a pure \ac{rest} \ac{api} contributes to a higher decoupling between clients and servers.
	  ...
	  As a consequence, it requires third applications to implement a uniform \ac{api} (i.e. \osapi{}) on top of them to allow our middleware consume their data.
  ...
  \item \emph{Making our data reusable by third applications}:
	...
	Although, any other application able to use \ac{http} and manage semantics can reuse data from our middleware,
	it is not hypertext-driven either. % con todo lo que ello conlleva, no quiero volver a aburrir al lector contandolo otra vez
...
Manageability helps to cope with a big number of devices allowing them to have an autonomous behavior. % specially in environments where they cannot be controlled through centrality
...
The ability to face mobility situations can be improved through the autonomies brought by space-based computing.
