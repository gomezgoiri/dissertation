...
\input{\pathchapfour/tables/coordination_properties}
...
\begin{description}
  \item[Performance] is divided into network performance, user-perceived performance and efficiency.
    \begin{itemize}
      ...
      \item \textbf{User-perceived performance} refers to the impact perceived by a user in front of an application. % UP Performance es una feature que no aporta mucho a nuestro caso
...
\input{\pathchapfour/tables/network_properties}
...
Whereas the devices part of the \ac{iot} are a subset of the ones present in \ac{ubicomp}, we observed that the challenges they have to cope with are the same ones.
For instance, smartphones are not part of the \ac{iot}, but cope with similar energy and computational limitations as the embedded devices. % posiblemente no todas las propiedades!
...
Furthermore, we also consider energy-related and computational constraints to be addressed by devices in \ac{ubicomp}. % ponerlo de forma que sea menos pegote?


% cómo las distintas propiedades vistas antes se combinan para aportar a estas
...
\input{\pathchapfour/tables/middleware_netprop_map}
...
In addition, in our \ac{http} \ac{api} the following issues remain unsolved:
...
The ability to cope with mobility situations can be improved through the decoupling brought by space-based computing.
...
\begin{itemize}
  ...
  \item Data consumers are able to process the responses they receive at least in one semantic format, e.g., \acs{n3} \citeweb{n32011} or RDF/XML \citeweb{rdfxml2004}. % por ejemplo :-P
\end{itemize}
...
\begin{itemize}
  \item The inference process on data providers.
        ...
        Still, most of the mobile and embedded devices are not able to process big amount of data. % TODO citar WoT2012
        Furthermore, reasoners are not available or optimized for many limited platforms.
...
The searching mechanism explained in the Chapter~\ref{cha:searching} takes into account the computing limitations of embedded and mobile devices.
...
As an example, Figure~\ref{fig:energy_consumption_tsc_section} shows the energy consumption on an embedded platform in an inactive period, whilst doing networking operations and when computing semantic data. % más detalles en la sección correspondiente
...
\begin{itemize}
  ...
  \item Regarding \ac{http}, its statelessness contributes to a bad \textbf{network performance}.
        In addition, the verboseness of most of the semantic formats also contributes negatively to that aspect.
\end{itemize}


\InsertFig{\pathchapfive/figures/PDF/energy_consumption.pdf}{fig:energy_consumption_tsc_section}{Energy consumption for an embedded platform}{
Energy consumption, measured in miliwatts, for a FoxG20 \citeweb{foxg20} during different activity periods.
}{0.6}{}
