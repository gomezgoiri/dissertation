...
The server off-loads work on the clients and frees resources quickly thanks to its statelessness.
This contributes to the coexistence of a huge number of components and interactions between them.

% Me había metido en berenjenales, pero al final he simplificado:
However, searching on the web (or in our model's \outerspace{}) unavoidably leads to scalability challenges on the search process. % porque tienes que preprocesar todo el contenido a buscar
The cause is that the search process needs to previously process all the web (or at least the subset where it is searched).
%    -> Esto es aún peor si los datos cambian con frecuencia
Furthermore, when the data searched changes frequently as in \ac{ubicomp}, these challenges grow exponentially.
...
This leads to each device to manage some additional piece of information from each server in the space.
To limit this negative impact, we limit the information written in the \outerspace{} using flat spaces. % i.e. groups of devices which share information

Regarding the \coordspace{}, this dissertation simply ignores how it deals with the scalability issues (e.g. by distributing the space).
