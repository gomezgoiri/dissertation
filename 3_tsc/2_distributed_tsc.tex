...
\begin{itemize}
  \item The \textbf{write_self} primitive writes a \emph{self-managed graph} and returns an \ac{uri} which identifies it.
...
However, we also presented the \emph{write_self} primitive.
\emph{Write_self} writes into the local device a untakeable graph (i.e. self-managed graph).
