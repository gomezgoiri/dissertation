...
%The \ac{rest} architectural style, defended by the \acf{wot} initiative, is probably the most prominent architectural solution used for smart-objects. % probably porque no tengo datos que lo defiendan
%The benefits that \ac{rest} brings to resource constrained devices have been deeply described in the previous chapter.

To that end, Section~\ref{sec:rest} analyzes the characteristics of the \ac{rest} architectural style and the similarities it shares with \ac{tsc}.
Later, it describes a \ac{rest} \ac{api} to access a triplespace. % TODO ver cómo escribir triplespace
...
Finally, Section~\ref{sec:middleware_qualitative_evaluation} evaluates the properties achieved with that model.
