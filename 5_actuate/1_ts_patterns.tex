...
First, we briefly explain \acl{ts}'s most common application patterns (see Section~\ref{sec:ts_patterns}). % according to the classification of \citet{freeman_javaspaces_1999}.
...
In this problem computation or a negotiation aspects are secondary. % se entenderá a qué me refiero con computation??
...
We considered the recommendation that states that an adult should walk at least 10.000 steps in an ordinary day \citep{tudor2002taking}.
...
\begin{itemize}
  ...
  \item Ease its adoption by any type of computing platform.
	This can be achieved by reducing the requirements on the \emph{clients}.
        For instance, a callback \ac{url} passed during the subscription can represent a minimal contract of the client with the server. % no viceversa
  ...
  \item Subscriptions from absent devices are useless and worsen efficiency.
	...
	Instead, more proactive mechanisms can be adopted by the device managing the space. % TODO buscar su nombre correcto
	For example, by letting subscriptions expire after a lifetime or removing them when it discovers the unavailability of a callback \ac{url}. % algo así como garbage collection
