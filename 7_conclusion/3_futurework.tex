...
\begin{description}
  ...
  \item[Reusing the data.] The semantic knowledge contributes to that goal.
			  ...
			  \begin{enumerate*}[label=\itshape(\arabic*\upshape)]
			    \item standardizing ontologies and using them (or at least when using own ontologies, referencing an upper ontology); and
			  ...
			  % semantica y cacharros pequeños, sigue siendo algo difícil. Necesidad de razonadores ligeros o específicos (citar al tipo del razonador para UbiComp).
\end{description}
...
However, for the future work, one can adopt \acs{sparql} thanks to an optional \ac{api}.
With such optional \ac{api}, the nodes should be aware that some of them are only able to interpret triple pattern templates.
...
\begin{itemize}
  ...
  \item Migration from a machine to another to avoid a single point of failure.
	%No node is essential so the system will still work.
        ...
        This would be analogous to the \ac{wp} role of our searching architecture. % mencionar Chapter~\ref{cha:searching}?
\end{itemize}
...
\subsection{Integration of the \restActuation{} in the Space}

Chapter~\ref{cha:actuate} presented an hybrid actuation technique which combines consumers using the \spaceActuation{} and \ac{rest} providers.
...
An option would be to translate the tasks written into the space to trigger an actuator change into goals.
